...
--   * 那個窄欄現在是「兩」張疊在一起的 canvas，不是一張 -- 領頭字和字身要兩種
--     不透明度，而一張 canvas 只有一個 alpha（理由見 canvas.lua 的 setAlpha）。
--     damage rect 沒有變大，但那塊區域上合成器要混的圖層從一層變兩層。這是為了
--     讓文字描邊能乾淨疊上去付的代價；上面那條 4% 只講面積，不再等於圖層工作量。
--
